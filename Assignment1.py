...
def ec2_lambda(event, context):
    neeraj_ec2 = boto3.client('ec2')

    # Describe instances with the 'Auto-Stop' and 'Auto-Start' tags
    auto_stop_instances = neeraj_ec2.describe_instances(Filters=[{'Name': 'tag:Action', 'Values': ['Auto-Stop']}])
    auto_start_instances = neeraj_ec2.describe_instances(Filters=[{'Name': 'tag:Action', 'Values': ['Auto-Start']}])

    # Stop instances with the 'Auto-Stop' tag
    for reservation in auto_stop_instances['Reservations']:
        for instance in reservation['Instances']:
            neeraj_ec2.stop_instances(InstanceIds=[instance['InstanceId']])

    # Start instances with the 'Auto-Start' tag
    for reservation in auto_start_instances['Reservations']:
        for instance in reservation['Instances']:
            neeraj_ec2.start_instances(InstanceIds=[instance['InstanceId']])
